...
export const invoiceFormSetAddProductDialogOpenState = (
  isAddProductDialogOpen
) =>
  reduxHelper.action(types.INVOICE_FORM_SET_ADD_PRODUCT_DIALOG_OPEN, {
    isAddProductDialogOpen
  });
...
export const invoiceFormSetSuggestions = ({ single, suggestions }) =>
  reduxHelper.action(types.INVOICE_FORM_SET_SUGGESTIONS, {
    single,
    suggestions
  });

export const invoiceFormSetSuggestion = ({ name, customerId, single }) =>
  reduxHelper.action(types.INVOICE_FORM_SET_SUGGESTION, {
    name,
    customerId,
    single
  });
...
export const invoiceFormAddNewProductToInvoice = (newInvoiceProduct) =>
  reduxHelper.action(
    types.INVOICE_FORM_ADD_NEW_PRODUCT_TO_INVOICE,
    newInvoiceProduct
  );

export const invoiceFormSetNewProduct = ({ newProduct, newInvoiceProduct }) =>
  reduxHelper.action(types.INVOICE_FORM_SET_NEW_PRODUCT, {
    newProduct,
    newInvoiceProduct
  });

export const invoiceFormRemoveProductFromInvoiceById = (invoiceProductId) =>
  reduxHelper.action(types.INVOICE_FORM_REMOVE_PRODUCT_FROM_INVOICE_BY_ID, {
    invoiceProductId
  });

export const invoiceFormSetInvoiceProductCount = ({
  invoiceProductId,
  count
}) =>
  reduxHelper.action(types.INVOICE_FORM_SET_INVOICE_PRODUCT_COUNT, {
    invoiceProductId,
    count
  });
...
export const invoiceFormSetDeliverAfterTimeUnit = (deliverAfterTimeUnit) =>
  reduxHelper.action(types.INVOICE_FORM_SET_DELIVER_AFTER_TIME_UNIT, {
